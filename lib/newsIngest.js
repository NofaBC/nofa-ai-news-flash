...
async function fetchMediastack(apiKey) {
  // Mediastack's `keywords` parameter has a long-standing, confirmed bug
  // where comma-separated values (e.g. "OpenAI,Anthropic,Gemini") are not
  // parsed as multiple OR'd terms - see
  // https://github.com/apilayer/mediastack/issues/3. A comma-joined list
  // of provider names silently matches nothing. Use a single broad,
  // comma-free phrase instead and let scoreArticle() do the precise
  // provider/infra-keyword filtering downstream, same as GNews results.
  const keywords = encodeURIComponent('artificial intelligence');
